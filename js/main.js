...
function createMovieCard(movieDetails) {
  const card = $("<div>").addClass(
    "col-xs-12 col-sm-6 col-md-4 col-lg-3 justify-content-center movie-card"
  );
  const cardContent = $("<div>").addClass("card mb-0 mr-0");
  const cardImage = $("<img>")
    .addClass("card-img-top")
    .attr("src", movieDetails.coverImageUrl)
    .attr("alt", movieDetails.title);
  const cardBody = $("<div>").addClass("card-body");
  const pillContainer = $("<div>").addClass("pill-container");

  movieDetails.genres.forEach((genre) => {
    const genrePill = $("<span>")
      .addClass("badge rounded-pill text-bg-dark")
      .text(genre);
    pillContainer.append(genrePill);
  });

  const cardTitle = $("<h5>")
    .addClass("library-card-title")
    .text(movieDetails.movieName);
  const viewButton = $("<button>")
    .addClass("btn btn-primary btn-gradient view-movie")
    .text("View movie");
  const saveButton = $("<button>")
    .addClass("btn btn-outline-dark save-movie")
    .html('<i class="bi bi-bookmark-heart-fill"></i>');
  ...
  viewButton.click(function () {
    window.location.href = `pages/individual.html?id=${movieDetails.id}`;
  });
